...
| No service-role/NVIDIA/Telegram/cron/TronGrid keys in the client bundle | PASS (code) | `SUPABASE_SERVICE_ROLE_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `NVIDIA_API_KEY`, `CRON_SECRET`, and `TRONGRID_API_KEY` are only ever read in server-only files: Server Components, Route Handlers (`runtime = "nodejs"`), and `lib/` modules imported exclusively by those (e.g. `lib/supabase/admin.ts`, `lib/telegram.ts`, `lib/telegram-settings.ts`, `lib/nvidia-client.ts`, plus the `/admin/settings` page and `bot-settings` API route added in QA-A2). None are imported by a `"use client"` component. |
| Rotate any key ever shared in chat/logs/screenshots | **ACTION NEEDED** | A third-party API token and a set of test-account credentials were shared in plaintext chat during an earlier working session for test-account setup. **Rotate them before public launch** if not already done — see the internal incident note (not duplicated here) for which ones specifically. |
